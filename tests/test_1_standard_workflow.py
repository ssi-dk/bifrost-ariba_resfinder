...
from bifrost_ariba_resfinder import launcher as barl
...
def test_install_component(mydb):
    ...
    args: argparse.Namespace = barl.parser(["--install"])
    barl.run_program(args)
...
def test_pipeline(mydb):
    ...
    args = barl.parser(["-id", sample_id,"-out", "/bifrost_test_data/output/test1"])
    barl.run_program(args)
    assert os.path.isfile("/bifrost_test_data/output/test1/ariba_resfinder/datadump_complete")